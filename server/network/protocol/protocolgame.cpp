...
void ProtocolGame::logout(bool displayEffect, bool forced) {
	...
	if (removePlayer && !player->isAccessPlayer()) {
		...
		if (tile && !tile->hasFlag(TILESTATE_PROTECTIONZONE) && player->hasCondition(CONDITION_INFIGHT)) {
			...
			// PIXELLOR CHANGE logout in fight
			 return;
		}
	}
...
void ProtocolGame::sendHighscores(const std::vector<HighscoreCharacter> &characters, uint8_t categoryId, uint32_t vocationId, uint16_t page, uint16_t pages, uint32_t updateTimer) {
	...
	for (const auto &[currentVocationId, vocationPtr] : vocationsMap) {
		...
		if (vocationPtr->getFromVocation() == static_cast<uint32_t>(currentVocationId)) {
			...
			++vocations;
			if (currentVocationBaseId == vocationId) {
	...
	for (const HighscoreCharacter &character : characters) {
		...
		msg.addString(character.loyaltyTitle.empty() ? "None": character.loyaltyTitle); // Character Loyalty Title
...
void ProtocolGame::sendBestiaryCharms() {
	...
	// PIXELLOR CHANGE custom price of charm reset config
	uint64_t resetAllCharmsCost = 100000 + (playerLevel > 100 ? playerLevel * g_configManager().getNumber(RESET_ALL_CHARM_PER_LVL_COST) : 0);
...
void ProtocolGame::sendOpenForge() {
	...
	for (const auto &item : player->getAllInventoryItems(true)) {
		...
		// PIXELLOR CHANGE - increasing max possible tier for 3,2,1 classification
		auto maxTier = (itemClassification == 4 ? maxConfigTier : (itemClassification * 2 - (itemClassification == 3 ? 1 : 0)));
	...
	*/
	...
	if (transferTotalCount > 0) {
		for (const auto &[itemId, tierAndCountMap] : donorTierItemMap) {
			...
			// PIXELLOR CHANGE - transfer does not validate slots
