...
namespace OXGaming.TibiaAPI.Network.ServerPackets
{
    public class CyclopediaMapData : ServerPacket
    {
        ...
        {
            DataType = message.ReadByte();
            if (DataType == (byte)CyclopediaMapDataType.MinimapMarker) {
                Position = message.ReadPosition();
                MinimapMarkerType = message.ReadByte();
                MinimapMarkerDescription = message.ReadString();
            } else if (DataType == (byte)CyclopediaMapDataType.DiscoveryData) {
            ...
            } else if (DataType == (byte)CyclopediaMapDataType.ActiveRaid) {
                Position = message.ReadPosition();
                ActiveRaidState = message.ReadByte(); // 0 = active (show exclamation mark on map) 
            } else if (DataType == (byte)CyclopediaMapDataType.ImminentRaidMainArea) {
                AreaId = message.ReadUInt16();
                NumberOfRaids = message.ReadByte();
            } else if (DataType == (byte)CyclopediaMapDataType.ImminentRaidSubArea) {
                AreaId = message.ReadUInt16();
                NumberOfRaids = message.ReadByte();
            } else if (DataType == (byte)CyclopediaMapDataType.SetDiscoveryArea) {
                AreaId = message.ReadUInt16();
            ...
            } else if (DataType == (byte)CyclopediaMapDataType.Passage) {
                Position = message.ReadPosition();
                PassageType = message.ReadByte(); // 0 = w/o subarea id, 1 = w/ subarea id?
                AreaId = message.ReadUInt16();
            } else if (DataType == (byte)CyclopediaMapDataType.SubAreaMonsters) {
            ...
            } else if (DataType == (byte)CyclopediaMapDataType.MonsterBestiary) {
                RaceId = message.ReadUInt16();
                IsUnlocked = message.ReadBool();
            } else if (DataType == (byte)CyclopediaMapDataType.Donations) {
            ...
            } else if (DataType == (byte)CyclopediaMapDataType.SetCurrentArea) {
                AreaId = message.ReadUInt16();
            } else {
